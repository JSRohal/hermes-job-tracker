...
const CalendarView = {
    ...
    apps.forEach(app => {
      (app.tasks || []).forEach(task => {
        if (task.dueDate) {
          push(task.dueDate, {
            ...
            company: app.company, position: app.position, appId: app.id,
            color: task.done ? "#64748b" : "#ec4899",
          });
        }
      });
      (app.timeline || []).forEach(ev => {
        if (ev.date) {
          ...
          push(ev.date, {
            kind: "event", label: ev.type, company: app.company, position: app.position, appId: app.id,
  ...
  // The chip's primary visible text — used both in the month grid and the
  // day-detail popup. Leads with company + position (not the status/task
  // label) specifically so that two applications to the same company for
  // different roles read as visibly different at a glance, not just on
  // hover — the status/task itself is still conveyed by color (+ icon for
  // tasks) and is always in the tooltip and day-detail view too.
  chipTitle(e) {
    return e.position ? `${e.company || "Untitled"} — ${e.position}` : (e.company || "Untitled");
  },
  chipTooltip(e) {
    return `${this.chipTitle(e)} · ${e.label}`;
  },
  ...
  render(container, apps, onDayClick) {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const chips = events.slice(0, MAX_VISIBLE).map(e => {
        ...
        return `
        <span class="cal-event-chip ${e.kind}${doneClass}" style="background:color-mix(in srgb, ${e.color} 22%, transparent); color:${e.color}" title="${escapeHtml(this.chipTooltip(e))}">${taskIcon}${escapeHtml(this.chipTitle(e))}</span>
      `;
      }).join("");
      cells += `
  ...
  upcomingReminders(apps) {
    ...
    apps.forEach(app => {
      (app.tasks || []).forEach(task => {
        if (task.done) return;
        items.push({ date: task.dueDate || "", label: task.title, company: app.company, position: app.position, appId: app.id, kind: "task" });
      });
      (app.timeline || []).forEach(ev => {
        if (!ev.date || ev.date <= today) return;
        items.push({ date: ev.date, label: ev.type, company: app.company, position: app.position, appId: app.id, kind: "event" });
      });
    });
