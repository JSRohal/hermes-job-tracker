...
const CalendarView = {
    ...
    apps.forEach(app => {
      (app.tasks || []).forEach(task => {
        if (task.dueDate) {
          push(task.dueDate, {
            ...
            company: app.company, appId: app.id, color: task.done ? "#475569" : "#f59e0b",
          });
        }
      });
      (app.timeline || []).forEach(ev => {
        if (ev.date) {
          ...
          push(ev.date, {
            kind: "event", label: ev.label, company: app.company, appId: app.id,
  ...
  render(container, apps, onDayClick) {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const dots = events.slice(0, 4).map(e => `<span class="cal-dot" style="background:${e.color}" title="${escapeHtml(e.label)}"></span>`).join("");
      cells += `
        <div class="cal-cell${isToday ? " today" : ""}${events.length ? " has-events" : ""}" data-date="${dateStr}">
          <div class="cal-daynum">${day}</div>
          <div class="cal-dots">${dots}</div>
          ${events.length > 4 ? `<div class="cal-more">+${events.length - 4}</div>` : ""}
        </div>`;
    }
  ...
  upcomingReminders(apps, withinDays = 14) {
    ...
    apps.forEach(app => {
      ...
      (app.timeline || []).forEach(ev => {
        ...
        if (d >= today && d <= limit) {
          items.push({ date: ev.date, label: ev.label, company: app.company, appId: app.id, kind: "event" });
        }
      });
    });
