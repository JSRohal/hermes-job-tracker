...
function renderDashboard() {
  ...
  const tiles = [
    ...
    { label: "This week", value: stats.perWeek },
  ...
  const upcoming = CalendarView.upcomingReminders(State.apps, 14);
  renderUpcomingList(document.getElementById("dashboard-upcoming"), upcoming);
}
...
function renderUpcomingList(container, items) {
  if (!items.length) {
    container.innerHTML = `<div class="chart-empty">Nothing on the horizon — you're clear for the next two weeks.</div>`;
    return;
  }
  container.innerHTML = items.map(item => `
    <div class="upcoming-item" data-app-id="${item.appId}">
      <span class="upcoming-date">${formatDate(item.date)}</span>
      <span class="upcoming-badge ${item.kind}">${item.kind === "task" ? "Task" : "Interview"}</span>
...
function wireApplicationsToolbar() {
  ...
  statusSelect.innerHTML = STATUS_PIPELINE.map(s => `<option value="${s.key}">${s.label}</option>`).join("");
  ...
  statusSelect.addEventListener("change", () => {
    State.filters.statuses = Array.from(statusSelect.selectedOptions).map(o => o.value);
    renderApplications();
  });
  ...
  bulkStatusSelect.addEventListener("change", () => {
    ...
    State.selection.forEach(id => {
      ...
      if (app) { app.status = key; addTimelineForStatus(app, key); Store.upsertApp(app); }
    });
...
function renderApplications() {
  ...
  container.querySelectorAll(".app-card").forEach(card => {
    ...
    statusSel.addEventListener("change", e => {
      ...
      addTimelineForStatus(app, e.target.value);
      Store.upsertApp(app);
      State.apps = Store.getApps();
      renderAll();
    });
...
function renderMiniTrack(currentKey) {
  const currentOrder = STATUS_MAP[currentKey].order;
  ...
  const openNodes = STATUS_PIPELINE.filter(s => !CLOSED_STATUSES.includes(s.key)).map(s => {
    const reached = s.order <= currentOrder && !isClosed;
    return `<span class="track-node ${reached ? "reached" : ""}" style="--dot-color:${s.color}" title="${s.label}"></span>`;
  }).join("");
  const closedNode = isClosed
    ? `<span class="track-node reached closed-node" style="--dot-color:${STATUS_MAP[currentKey].color}" title="${STATUS_MAP[currentKey].label}"></span>`
    : "";
  return openNodes + closedNode;
}

function addTimelineForStatus(app, statusKey) {
  app.timeline = app.timeline || [];
  const label = STATUS_PIPELINE.find(s => s.key === statusKey)?.label || statusKey;
  const already = app.timeline.some(t => t.statusKey === statusKey && t.date === todayISO());
  if (!already) {
    app.timeline.push({ id: uid(), type: label, statusKey, date: todayISO(), notes: "" });
  }
  if (statusKey === "applied" && !app.dateApplied) app.dateApplied = todayISO();
}
...
function renderOverviewTab(container) {
  ...
  bind("#f-status", "change", e => { d.status = e.target.value; addTimelineForStatus(d, e.target.value); });
  bind("#f-date", "input", e => d.dateApplied = e.target.value);
...
function renderTimelineTab(container) {
  ...
  container.innerHTML = `
      ...
      ${items.length ? items.map(ev => `
        <div class="tab-list-item" data-id="${ev.id}">
          <div class="tli-main">
            ...
            <strong>${escapeHtml(ev.type || ev.label || "Event")}</strong>
    ...
    <fieldset class="fieldset">
      ...
      <div class="form-grid">
        <label>Type
          <select id="tl-type">${TIMELINE_EVENT_TYPES.map(t => `<option value="${t}">${t}</option>`).join("")}</select>
        </label>
        <label>Date
      ...
      <button type="button" class="btn-secondary" id="tl-add">Add to timeline</button>
    </fieldset>
  `;
  ...
  container.querySelector("#tl-add").addEventListener("click", () => {
    const type = container.querySelector("#tl-type").value;
    const date = container.querySelector("#tl-date").value;
    const notes = container.querySelector("#tl-notes").value;
    const statusKey = TIMELINE_TYPE_STATUS_MAP[type] || null;
    d.timeline = d.timeline || [];
    d.timeline.push({ id: uid(), type, statusKey, date, notes });
    if (statusKey) { d.status = statusKey; if (statusKey === "applied" && !d.dateApplied) d.dateApplied = date; }
    renderTimelineTab(container);
  });
}
    ...
    <div class="form-grid">
      <label>Resume version used
        <select id="doc-resume-version">
          ...
          ${resumeVersions.map(v => `<option value="${escapeAttr(v)}" ${d.resumeVersion === v ? "selected" : ""}>${escapeHtml(v)}</option>`).join("")}
        </select>
      </label>
      <label>Cover letter version used
        <select id="doc-cover-version">
          <option value="">— none selected —</option>
          ${coverVersions.map(v => `<option value="${escapeAttr(v)}" ${d.coverLetterVersion === v ? "selected" : ""}>${escapeHtml(v)}</option>`).join("")}
        </select>
      </label>
    </div>
    <p class="hint">Manage the master list of versions in Settings → Resume &amp; cover letter library.</p>
...
function wireCalendar() {
  document.getElementById("export-ics-btn").addEventListener("click", () => {
    ...
    downloadFile("trackline-calendar.ics", ics, "text/calendar");
...
function renderCalendarView() {
  ...
  renderUpcomingList(document.getElementById("calendar-upcoming"), CalendarView.upcomingReminders(State.apps, 14));
}
...
function openDayModal(dateStr, events) {
  ...
  if (!events.length) {
    ...
    body.innerHTML = events.map(e => `
      <div class="upcoming-item" data-app-id="${e.appId}">
        <span class="upcoming-badge ${e.kind}">${e.kind === "task" ? "Task" : "Event"}</span>
...
function wireSettings() {
  ...
  document.getElementById("export-json-btn").addEventListener("click", () => {
    downloadFile(`trackline-backup-${todayISO()}.json`, Store.exportJSON(), "application/json");
  ...
  document.getElementById("import-json-input").addEventListener("change", async e => {
    ...
    try {
      ...
      toast("Couldn't read that file — is it a valid Trackline backup?");
    }
    e.target.value = "";
  });
...
function removeLibraryItem(settingsKey, val) {
  const list = (State.settings[settingsKey] || []).filter(v => v !== val);
  Store.updateSettings({ [settingsKey]: list });
  State.settings = Store.getSettings();
  renderSettingsPanel();
}
...
function renderSettingsPanel() {
  ...
  renderChipEditor("resume-lib-list", State.settings.resumeVersions, "resumeVersions");
  renderChipEditor("cover-lib-list", State.settings.coverLetterVersions, "coverLetterVersions");
