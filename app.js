...
function wireApplicationsToolbar() {
  ...
  bulkStatusSelect.addEventListener("change", () => {
    ...
    State.selection.forEach(id => {
      ...
      if (app) { logStatusEvent(app, key); Store.upsertApp(app); }
    });
...
function renderApplications() {
  ...
  container.querySelectorAll(".app-card").forEach(card => {
    ...
    statusSel.addEventListener("change", e => {
      const app = Store.getApp(id);
      logStatusEvent(app, e.target.value);
      Store.upsertApp(app);
      State.apps = Store.getApps();
      renderAll();
    });
...
// Logging a status/event is now a single unified action: it records a
// timeline entry (for history + the calendar) AND sets the application's
// current status. A specific date can be passed for a past/future-dated
// entry (e.g. "Final Interview next Friday"); quick actions that have no
// date context (card dropdown, bulk edit, Kanban drag) just use today.
// The statusKey+date de-dupe check prevents a duplicate entry landing on
// today's date when a dated log already covers that exact day.
function logStatusEvent(app, statusKey, date = todayISO(), notes = "") {
  app.timeline = app.timeline || [];
  const label = STATUS_MAP[statusKey]?.label || statusKey;
  const already = app.timeline.some(t => t.statusKey === statusKey && t.date === date);
  if (!already) {
    app.timeline.push({ id: uid(), type: label, statusKey, date, notes });
  }
...
function renderOverviewTab(container) {
  ...
  bind("#f-status", "change", e => logStatusEvent(d, e.target.value));
...
function renderTimelineTab(container) {
  ...
  container.innerHTML = `
    ...
    <fieldset class="fieldset">
      ...
      <p class="hint tight">Logging an event sets this application's status to match and records it on the calendar. Date it in the past or future as needed — today's status change won't be duplicated if it matches an existing entry for that date.</p>
      <div class="form-grid">
        <label>Status / event
  ...
  container.querySelector("#tl-add").addEventListener("click", () => {
    ...
    logStatusEvent(d, statusKey, date, notes);
    renderTimelineTab(container);
...
function renderInterviewNotesTab(container) {
  ...
  container.innerHTML = `
    <fieldset class="fieldset">
      ...
      <p class="hint tight">Manually entered — Hermes runs fully offline and doesn't call out to Glassdoor or any other service.</p>
      <div class="form-grid">
    ...
    <fieldset class="fieldset">
      ...
      ${match ? `
        <div class="match-summary">
          <span class="stars" title="Auto-computed match">${"★".repeat(match.score)}${"☆".repeat(5 - match.score)}</span>
      ...
      ` : `<p class="hint tight">Select a resume version with saved text and paste a job description on Overview, then analyze the match.</p>`}
