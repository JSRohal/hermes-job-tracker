...
function addTimelineForStatus(app, statusKey) {
  ...
  const already = app.timeline.some(t => t.statusKey === statusKey && t.date === todayISO() && t.source === "status-change");
  if (!already) {
    // source: "status-change" marks this as an automatic history entry, not
    // a user-set reminder — keeps it out of the "upcoming reminders" lists.
    app.timeline.push({ id: uid(), type: label, statusKey, date: todayISO(), notes: "", source: "status-change" });
  }
...
function renderTimelineTab(container) {
  ...
  container.innerHTML = `
    ...
    <fieldset class="fieldset">
      ...
      <p class="hint tight">Logs history and (for interview-type events) an upcoming reminder — it won't change this application's status. Use the status field on Overview for that.</p>
      <div class="form-grid">
        <label>Type
  ...
  container.querySelector("#tl-add").addEventListener("click", () => {
    ...
    // Logging an event is a history/reminder entry only — it never changes
    // the application's actual status. Use the Overview tab's status field
    // (or the card's quick-status dropdown) to change status.
    d.timeline.push({ id: uid(), type, statusKey, date, notes, source: "manual" });
    renderTimelineTab(container);
  });
}
  ...
  board.querySelectorAll(".kanban-col-body").forEach(col => {
    ...
    col.addEventListener("drop", e => {
      ...
      if (app && app.status !== newStatus) {
        ...
        addTimelineForStatus(app, newStatus);
        Store.upsertApp(app);
        State.apps = Store.getApps();
        renderKanbanView();
