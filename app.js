...
// Handy for applying to a second role at a company you're already tracking
// (exactly the "multiple positions, same company" case) — carries over
// company-level context (location, company intel, tags, resume/cover
// picks) but starts fresh on everything specific to a single application:
// position, status, dates, timeline, checklist, notes, and the job posting
// itself.
function duplicateApplication(id) {
  const original = Store.getApp(id);
  if (!original) return;
  const copy = emptyApplication({
    company: original.company,
    location: original.location,
    tags: [...(original.tags || [])],
    resumeVersion: original.resumeVersion || "",
    coverLetterVersion: original.coverLetterVersion || "",
    salary: { ...(original.salary || { min: "", max: "", currency: "USD", period: "year" }) },
    companyIntel: { ...(original.companyIntel || { hq: "", size: "", interviewDifficulty: "Unknown" }) },
  });
  Store.upsertApp(copy);
  State.apps = Store.getApps();
  renderAll();
  toast(`Duplicated ${original.company || "application"} — fill in the new role`);
  openAppModal(copy.id);
}
...
function renderUpcomingList(container, items) {
  ...
  container.innerHTML = items.map(item => `
    <div class="upcoming-item" data-app-id="${item.appId}" data-kind="${item.kind}">
      ...
      <span class="upcoming-company muted">${escapeHtml(item.company)}${item.position ? ` — ${escapeHtml(item.position)}` : ""}</span>
    </div>`).join("");
  container.querySelectorAll(".upcoming-item").forEach(el => {
    el.addEventListener("click", () => openAppModal(el.dataset.appId, el.dataset.kind === "task" ? "tasks" : "timeline"));
  });
}
...
function renderApplications() {
  ...
  container.querySelectorAll(".app-card").forEach(card => {
    ...
    card.addEventListener("click", (e) => {
      if (e.target.closest(".card-checkbox, .fav-star, .card-status-select, .card-delete-btn, .card-open-url, .card-duplicate-btn")) return;
      openAppModal(id);
    });
  });
  ...
  return `
    <article class="app-card" data-id="${app.id}" style="--status-color:${status.color}">
      ...
      <div class="card-actions">
        ${app.jobUrl ? `<a class="btn-ghost card-open-url" href="${escapeAttr(app.jobUrl)}" target="_blank" rel="noopener" title="Open job posting">Open posting ↗</a>` : ""}
        <button class="btn-ghost card-duplicate-btn" title="Duplicate for another role at this company">Duplicate</button>
...
// "Applied" is semantically singular — an application has exactly one real
// apply date — so instead of treating it like other repeatable timeline
// entries, logging it always updates the existing entry (and dateApplied)
// rather than only filling in dateApplied when it happened to be empty.
function recordAppliedDate(app, date, notes = "") {
  app.timeline = app.timeline || [];
  const existing = app.timeline.find(t => t.statusKey === "applied");
  if (existing) {
    existing.date = date;
    if (notes) existing.notes = notes;
  } else {
    app.timeline.push({ id: uid(), type: STATUS_MAP.applied.label, statusKey: "applied", date, notes });
  }
...
function setStatus(app, statusKey) {
  ...
  if (statusKey === "applied") recordAppliedDate(app, date);
  else logTimelineEntry(app, statusKey, date, "");
  app.status = statusKey;
}

function logHistoricalEvent(app, statusKey, date, notes) {
  if (statusKey === "applied") recordAppliedDate(app, date, notes);
  else logTimelineEntry(app, statusKey, date, notes);
  const newOrder = STATUS_MAP[statusKey].order;
  const currentOrder = STATUS_MAP[app.status].order;
  if (newOrder >= currentOrder) app.status = statusKey;
}
...
// Escape closes whatever modal is topmost (confirm > day-detail > editor,
// since a confirm dialog can be stacked on top of the editor). "/" jumps to
// the search box on the Applications view — only when not already typing in
// a field, so it doesn't hijack normal typing.
function wireKeyboardShortcuts() {
  document.addEventListener("keydown", e => {
    if (e.key === "Escape") {
      if (!document.getElementById("confirm-modal").hidden) { closeModal("confirm-modal"); return; }
      if (!document.getElementById("day-modal").hidden) { closeModal("day-modal"); return; }
      if (!document.getElementById("app-modal").hidden) { maybeCloseAppModal(); return; }
      return;
    }
    if (e.key === "/" && !e.metaKey && !e.ctrlKey) {
      ...
      if (isTyping) return;
      const allModalsHidden = document.getElementById("app-modal").hidden && document.getElementById("day-modal").hidden && document.getElementById("confirm-modal").hidden;
      if (!allModalsHidden) return;
      e.preventDefault();
...
function openAppModal(id, initialTab = "overview") {
  if (id) {
  ...
  document.querySelectorAll("#modal-tabs .tab-btn").forEach(b => b.classList.toggle("active", b.dataset.tab === initialTab));
  renderModalBody();
  openModal("app-modal");
}
...
function renderOverviewTab(container) {
  ...
  container.innerHTML = `
    ...
    <p class="hint tight">The date you actually applied is tracked automatically from the Timeline tab — log an "Applied" event there (backdated if needed) instead of typing a date here.</p>
...
function renderInterviewNotesTab(container) {
  ...
  const intel = d.companyIntel || (d.companyIntel = { hq: "", size: "", interviewDifficulty: "Unknown" });
...
function openDayModal(dateStr, events) {
  ...
  if (!events.length) {
    ...
    body.innerHTML = events.map(e => {
      ...
      return `
      <div class="upcoming-item${doneClass}" data-app-id="${e.appId}" data-kind="${e.kind}">
        <span class="upcoming-badge ${e.kind}">${taskIcon}${e.kind === "task" ? "Task" : "Event"}</span>
        <span class="upcoming-label">${escapeHtml(e.label)}</span>
        <span class="upcoming-company muted">${escapeHtml(e.company || "Untitled")}${e.position ? ` — ${escapeHtml(e.position)}` : ""}</span>
      </div>`;
    }).join("");
    body.querySelectorAll(".upcoming-item").forEach(el => {
      el.addEventListener("click", () => { closeModal("day-modal"); openAppModal(el.dataset.appId, el.dataset.kind === "task" ? "tasks" : "timeline"); });
    });
  }
  openModal("day-modal");
}
