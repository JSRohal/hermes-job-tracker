...
const Analytics = {
  ...
  tokenizeRaw(text) {
    return (text || "")
      .toLowerCase()
      .replace(/['\u2019]/g, "") // delete apostrophes so "bachelor's" -> "bachelors" (one token), not "bachelor" + "s"
      .replace(/[^a-z0-9+.#/&\- ]/g, " ")
      .split(/\s+/)
  ...
  // "...Six Sigma. CNC machining..." must never produce "sigma cnc
  // machining", and "Python, Java, SQL" must never produce "python java").
  // Splits on ". " / "; " / ", " / bullets / blank-line paragraph breaks —
  // but a single newline with no bullet marker is treated as a soft wrap
  // (just becomes a space) rather than a hard boundary, since resumes
  // pasted from a PDF very often word-wrap mid-phrase ("...finite\nelement
  // analysis...") and a real compound term shouldn't be split apart by that.
  splitSentences(text) {
    const withForcedBreaks = (text || "").replace(/\n(?=\s*(?:[•\-*]|\d+[.)])\s)/g, "\n\n");
  ...
  // True if the term IS a blocked fluff phrase, or is wholly contained
  // within one (catches fragments like "wear multiple" or "multiple hats"
  // that are sub-slices of the blocked trigram "wear multiple hats" —
  // blocking the exact phrase alone isn't enough since bigram/trigram
  // extraction also produces its shorter sub-phrases as separate terms).
  isFluffPhrase(term) {
    if (MATCH_FLUFF_PHRASES.has(term)) return true;
    if (MATCH_SKILL_DICTIONARY.has(term)) return false; // never suppress a known real skill term
    for (const fluff of MATCH_FLUFF_PHRASES) {
      if (fluff.length > term.length && fluff.includes(term)) return true;
    }
    // Also blocks a multi-word candidate built entirely from words that are
    // EACH already their own recognized fluff phrase — e.g. "detail-oriented
    // self-starter" combines two separately-blocked hyphenated fluff terms;
    // hyphens are kept as part of a single token, so this pairing wouldn't
    // otherwise be caught by the substring check above.
    const words = term.split(" ");
    if (words.length >= 2 && words.every(w => MATCH_FLUFF_PHRASES.has(w))) return true;
    return false;
  },
  ...
  extractWeightedTerms(text) {
    ...
    const add = (term, weight) => {
      ...
      if (!norm || MATCH_STOPWORDS.has(norm) || this.isFluffPhrase(norm)) return;
  ...
  computeMatch(resumeText, jdText) {
    ...
    // Two-tier keyword selection: real, known skill/tool terms (Tier 1)
    // always count fully — a JD only mentioning "SolidWorks" once shouldn't
    // lose to five incidental mentions of "team". Tier 2 (everything else,
    // ranked by weight) only fills in the remaining slots up to a sensible
    // minimum. Tier 2 is also capped at bigrams — an unmatched trigram is
    // overwhelmingly likely to be an accidental word-adjacency artifact
    // (JD prose rarely delimits every clause with punctuation, so 3-word
    // windows often bridge two unrelated phrases) rather than a real term;
    // dictionary trigrams like "finite element analysis" still get through
    // via Tier 1's exact lookup, which is a much more reliable signal.
    const allTerms = Object.entries(jdWeights);
    const tier1 = allTerms.filter(([term]) => MATCH_SKILL_DICTIONARY.has(term)).sort((a, b) => b[1] - a[1]);
    const tier2 = allTerms
      .filter(([term]) => !MATCH_SKILL_DICTIONARY.has(term) && term.split(" ").length <= 2)
      .sort((a, b) => b[1] - a[1]);
    const MIN_KEYWORDS = 15, MAX_KEYWORDS = 40;
    const ranked = tier1.concat(tier2).slice(0, Math.max(MIN_KEYWORDS, Math.min(MAX_KEYWORDS, tier1.length + 10)));
