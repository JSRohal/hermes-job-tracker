...
const Analytics = {
  ...
  // Fully offline — no APIs, no ML model download. The previous version was
  // naive raw-frequency unigram overlap, which surfaces whatever words repeat
  // most (often boilerplate) rather than what's actually required, and loses
  // multi-word skills entirely ("machine learning" ≠ "machine" + "learning").
  // This version: extracts phrases (bigrams) as well as single words, weighs
  // terms found near a "Requirements/Qualifications"-style heading much more
  // than incidental text, and gives known skill/tool terms from a curated
  ...
  extractWeightedTerms(text) {
    ...
    source.forEach(section => {
      ...
      const words = this.tokenize(section);
      words.forEach(w => add(w, sectionWeight));
      for (let i = 0; i < words.length - 1; i++) {
        add(`${words[i]} ${words[i + 1]}`, sectionWeight * 1.6); // phrases are more specific signal than lone words
      }
    });
  ...
  computeMatch(resumeText, jdText) {
    ...
    const resumeWords = this.tokenize(resumeText);
    const resumeTerms = new Set(resumeWords);
    for (let i = 0; i < resumeWords.length - 1; i++) resumeTerms.add(`${resumeWords[i]} ${resumeWords[i + 1]}`);

    const ranked = Object.entries(jdWeights).sort((a, b) => b[1] - a[1]).slice(0, 40);
    if (!ranked.length || !resumeTerms.size) {
    ...
    ranked.forEach(([term, weight]) => {
      if (resumeTerms.has(term)) { matchedWeight += weight; present.push(term); }
  ...
  generateInsights(apps) {
    ...
    // 9) Not enough data yet — say so plainly instead of guessing
    if (!insights.length) {
